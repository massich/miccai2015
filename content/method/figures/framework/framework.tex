...
%% Draw the links
  ...
  \tikzstyle{p}=[draw=gray, line width=8pt, -triangle 45]
  \path[p] (bwImage)    --   (map);
  \path[p] (xx)         |-   (features.west);
  \path[p] (map.east)   -|   (mrf.north);
  \path[p] (svm.east)   -|   (min.north);
  \path[p] (min)        --   (remap);
  \path[p] (mrf.east)   -|   (min.south);
  \path[p] (map.east)   -|   node (pointmap){\normalPic{\pointMapping}}   (features.south);
  \path[p] (features)   --   node {\labelPic{\classificableSample}}       (svm);
  \path[p] (remap)      --   node {\labelPic{\spMapping}}                 (output);
