...
%% Drawing the Nodes
  ...
  % SVM
  \node[right=50pt of features, label={below:Classifier}]
    (svm){\normalPic{\classBlock}};
...
%% Draw the links
  ...
  \tikzstyle{p}=[draw=gray, line width=3pt, -latex]
  ...
  \path[p]   (features)  --  node[l,inner sep=0pt](table){$|\mathcal{S}|\times|\mathcal{F}|$}    (svm);
...
%% Draw the blocks
  \node[fill=white, anchor=west, text width=3cm, yshift=15pt] at (bwImage.west|-features.north)
  ...
  \coordinate[xshift=-5pt] (a) at (stage_1.west);
  \coordinate[yshift=-10pt] (aa) at (table.east|-mrf.south);
  \node[fit=(a)(aa), draw, dashed, very thin]{};
  \coordinate[xshift=5pt] (b) at (a-|aa);
  \coordinate[xshift=-5pt] (bb) at (aa-|remap.west);
  \node[fit=(b)(bb), draw, dashed, very thin]{};
  \coordinate[xshift=5pt] (c) at (b-|bb);
  \coordinate[xshift=5pt] (cc) at (bb-|output.east);
